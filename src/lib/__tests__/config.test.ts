...
describe('config', () => {
  ...
  describe('isDev', () => {
    it('reflects NODE_ENV', () => {
      // Since isDev is a constant exported from the module, and Bun caches modules,
      // we check it matches the current NODE_ENV it was initialized with.
      const expected = process.env.NODE_ENV !== 'production';
      expect(config.isDev).toBe(expected);
    });
  });
