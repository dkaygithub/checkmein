...
describe('Performance benchmark for processPostEventEmails', () => {
    ...
    it('benchmarks processPostEventEmails', async () => {
        ...
        prisma.event.findMany = jest.fn().mockImplementation(async () => {
            if (callCount === 0) {
                callCount++;
                return mockEvents as unknown as ReturnType<typeof prisma.event.findMany>;
            }
            return [];
        });

        prisma.event.update = jest.fn().mockImplementation(async () => { return {} as unknown as ReturnType<typeof prisma.event.update>; });

        let uniqueCalls = 0;
        prisma.participant.findUnique = jest.fn().mockImplementation(async ({ where }: { where: { id: number } }) => {
            uniqueCalls++;
            await new Promise(resolve => setTimeout(resolve, 5));
            return { email: `lead${where.id}@example.com` } as unknown as ReturnType<typeof prisma.participant.findUnique>;
        });
        ...
        prisma.participant.findMany = jest.fn().mockImplementation(async ({ where }: { where: { id: { in: number[] } } }) => {
            ...
            return ids.map(id => ({ id, email: `lead${id}@example.com` })) as unknown as ReturnType<typeof prisma.participant.findMany>;
        });
